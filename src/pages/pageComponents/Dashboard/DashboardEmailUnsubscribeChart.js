...
import DatePickerMonthSelectionFDEUnsubscribe from './DatePickerMonthSelectionFDEUnsubscribe'
export default class DashboardEmailUnsubscribeChart extends React.Component {
  ...
  render () {
    console.log('this.props inside DashboardEmailUnsubscribeChart', this.props)
    const {
      analysisData,
      analysisLoader,
      user,
    ...
    let analysisGraphData = this.transformAnalysisData({
        analysisData,
        key: 'totalUnsubscribe',
        label: 'Unsubscribe Email User',
        chartName: 'bar',
      })
    ...
    let chartContent =  analysisData.length ? <Bar
        ...
        height={400}
      /> : <h2>No Unsubscribe email user found</h2>

    let chart = analysisLoader ? <div className="text-center" > <Spin /> </div> : chartContent
    return (
      <Card className='my-2' title={<DatePickerMonthSelectionFDEUnsubscribe title="Email Unsubscribe" user={user} fetchEmailUnsubscribeAnalysis={fetchEmailUnsubscribeAnalysis} />} >
      {chart}
      </Card>
      )
