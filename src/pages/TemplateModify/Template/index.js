/**
 difference between templateCreate and modifyTemplate

* copied all content from template create page
* componentDidMount copied from old templateModify page
* transformExcludeOrders copied from old templateModify page
* changed action call in form tag onSubmit and Done button _submitTemplateModifyDetails instead of _submitTemplateCreationDetails
* _submitTemplateModifyDetails copied from old templateModify page
* added    attachments_preview: "", in state
* import SERVER_STATIC_URL
* Using TemplateModifyPreviewModal
  sendTestMail: payload => dispatch(sendTestMail(payload))
...
import { fetchTemplates, tCreationSubmit, tModifySubmit, tDeleteSubmit, sendTestMailForTemplateEdit  } from "./../../../ducks/template";
...
import TemplateModifyPreviewModal from '../../pageComponents/Template/TemplateModifyPreviewModal'
...
const mapDispatchToProps = (dispatch, props) => ({
  ...
  sendTestMailForTemplateEdit: payload => dispatch(sendTestMailForTemplateEdit(payload))
});
...
class TemplateComponents extends React.Component {
  ...
  _submitTemplateModifyDetails = () => {
    ...
    message.info("Updating template");
  ...
  componentDidMount = () => {
    const {fetchOrders, fetchTemplates, fetchProducts, user: {_id: userId}} = this.props
    fetchOrders(userId)
      ...
      if (self.props.templates && self.props.templates.length) {
        ...
        if (typeof templateData === "object" && Object.keys(templateData).length) {
          ...
          self.setState({
            ...
            minimum_item_condition,
            fulfillment_type,
            template_output: email_message,
            messageBody: email_message,
            companyLogoFilePreview: logoPreview,
            exclude_orders: exclude_orders_arr,
            include_products: include_products ? include_products : '',
            include_products_type: include_products_type ? include_products_type : 'asin',
            exclude_products: exclude_products ? exclude_products : '',
            exclude_products_type: exclude_products_type ? exclude_products_type : 'asin',
            attachments,
  ...
  render() {
    const modifyTemplateId = this.props.match.params.id;
    let templateData = this.props.templates.filter((temp) => temp["_id"] === modifyTemplateId)[0];
      ...
      <div className="card">
        <div className="card-body">
...

          {
            ...
            <div style={spinStyle}>
              <Spin tip="Fetching data to modify template" />
            </div> :
          ...
          <div className="steps-action">
            ...
            }
            {
              ...
              {
                submitApiCall ? null :
                <TemplateModifyPreviewModal
                  template_name={template_name}
                  templateData={templateData}
                  orders={this.props.orders}
                  template_output={this.state.template_output}
                  attachments={this.state.attachments.length ? this.state.attachments : null}
                  companyLogoFilePreview={this.state.companyLogoFilePreview}
                  sendTestMailForTemplateEdit={this.props.sendTestMailForTemplateEdit}
                 />
              }
